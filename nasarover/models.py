...
class Rover(models.Model):
    ...
    def change_dirn(self, dirn):
        ...
        if dirn == 'R':
            ...
            temp_dirn = temp_r.dirn
            temp = direction[(direction.find(temp_dirn) + 1) % 4]
            temp_r.dirn = temp
            temp_r.save()
        else:
            temp_r = RoverPos.objects.get(r_id=self.id)
            temp_dirn = temp_r.dirn
            temp = direction[(direction.find(temp_dirn) - 1)]
            temp_r.dirn = temp
            temp_r.save()
    ...
    def investigate(self):
        ...
        for i in r_sen:
            ...
            sg = SubGrid.objects.get(\
                g_id=rover_pos.g_id, pos_x=rover_pos.pos_x, \
                pos_y=rover_pos.pos_y)
